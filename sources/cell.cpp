...
cell_option_book weekend::call_option_special(hero* _hero, int day){
    ...
    options.push_back(cell_option("공부하기", [&_hero, day](){
        ...
        { .energy = _hero->get_energy_consuming_rate(), .study = -2, .relationship = -2, .self_develop = 0, .love = 0, .stress = 9 },
        day);
        return "날씨가 너무 화창한바람에 공부가 되지 않는다...괜히 시간만날렸다.";
    }));
    options.push_back(cell_option("술마시기", [&_hero, day](){
        _hero->change_status(
        { .energy = _hero->get_energy_consuming_rate(), .study = -4, .relationship = 5, .self_develop = 0, .love = 1, .stress = 6 },
    ...
    options.push_back(cell_option("집가기", [&_hero, day](){
        _hero->change_status(
        { .energy = 8, .study = 0, .relationship = 0, .self_develop = 0, .love = 0, .stress = 3 },
        day);
        return "..알고보니 나는 제주도 사람이었다... 비행기가 결항되어 집에 갈 수 없다....";
    }));
    return cell_option_book(options);
}
...
cell_option_book march::call_option(hero* _hero, int day){
    ...
    options.push_back(cell_option("밥사달라고하기", [&_hero, day](){
        _hero->change_status(
...
cell_option_book special::call_option(hero* _hero, int day) {
    ...
    options.push_back(cell_option("참가한다", [&_hero, day]() {
        _hero->change_status(
        { .energy = _hero->get_energy_consuming_rate(), .study = -4, .relationship = 8, .self_develop = 0, .love = 0, .stress = 4 },
        day);
    }));
    options.push_back(cell_option("참가하지 않는다", [&_hero, day]() {
        _hero->change_status(
...
cell_option_book eve_1::call_option(hero* _hero, int day) {
    ...
    options.push_back(cell_option("수업가기", [&_hero, day]() {
        _hero->change_status(
    ...
    options.push_back(cell_option("나들이", [&_hero, day]() {
        _hero->change_status(
...
cell_option_book dance::call_option(hero* _hero, int day) {
    ...
    options.push_back(cell_option("춤 연습하기", [&_hero, day]() {
        _hero->change_status(
