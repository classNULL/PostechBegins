...
cell_option_book cell::call_option_special(hero* _hero, int day) {
  ...
  options.push_back(cell_option(L"공부하기", [_hero, day]() {
    _hero->change_status({.energy = _hero->get_energy_consuming_rate(),
                          .love = 0,
                          .relationship = -2,
                          .self_develop = 0,
                          .study = -2,
                          .stress = 7},
                         day);
    return L"날씨가 너무 화창한 바람에 공부가 되지 않는다...괜히 시간만 "
           L"날렸다.";
  }));
  options.push_back(cell_option(L"술마시기", [_hero, day]() {
    _hero->change_status({.energy = _hero->get_energy_consuming_rate(),
                          ...
                          .stress = 5},
                         day);
    return L"....실연당한 친구의 이야기를 들어주며 술을 마셨다..같은 이야기만 "
           L"38번들었다..괜히 시간만 날렸다..";
  }));
  options.push_back(cell_option(L"동아리 활동하기", [_hero, day]() {
    _hero->change_status(
        {.energy = _hero->get_energy_consuming_rate(),
         .love = 1,
         .relationship = 3,
         .self_develop = 2,
         .study = -4,
         .stress = -3},
        _hero->get_title_book().get_total_title_effect().circle, day);
  ...
  options.push_back(cell_option(L"휴식", [_hero, day]() {
    _hero->change_status({.energy = 8,
                          ...
                          .stress = -2},
                         day);
    return L"..퀴즈 점수가 나왔다..나는 먼지같은 인간이다..이 상황에 쉴 생각을 "
           L"하다니.. 쉬어도 쉰 것 같지 않다...";
  }));
...
cell_option_book normal::call_option(hero* _hero, int day) {
  if (_hero->exhausted()) return this->call_option_zero(_hero, day);

  if (rand() % 100 == 1) return this->call_option_special(_hero, day);
  ...
  options.push_back(cell_option(L"술마시기", [_hero, day]() {
    _hero->change_status(
        {.energy = _hero->get_energy_consuming_rate(),
         ...
         .study = -4,
         .stress = 3},
        _hero->get_title_book().get_total_title_effect().alcohol, day);
...
cell_option_book weekend::call_option_special(hero* _hero, int day) {
  ...
  options.push_back(cell_option(L"휴식", [_hero, day]() {
    _hero->change_status({.energy = 8,
                          ...
                          .stress = -2},
                         day);
    return L"..퀴즈 점수가 나왔다.. 나는 먼지같은 인간이다.. 이 상황에 쉴 "
           L"생각을 하다니.. 쉬어도 쉰 것 같지 않다...";
  }));
  options.push_back(cell_option(L"집가기", [_hero, day]() {
    _hero->change_status({.energy = 8,
                          .love = 0,
                          ...
                          .stress = 3},
                         day);
    return L"..알고보니 나는 제주도 사람이었다... 비행기가 결항되어 집에 갈 수 "
           L"없다....";
  }));
  return cell_option_book(options);
}
  ...
  options.push_back(cell_option(L"술마시기", [_hero, day]() {
    _hero->change_status(
        {.energy = _hero->get_energy_consuming_rate(),
         ...
         .study = -4,
         .stress = 4},
        _hero->get_title_book().get_total_title_effect().alcohol, 1);
  ...
  options.push_back(cell_option(L"동아리 활동하기", [_hero, day]() {
    _hero->change_status(
        {.energy = _hero->get_energy_consuming_rate(),
         ...
         .study = -4,
         .stress = 1},
        _hero->get_title_book().get_total_title_effect().circle, 1);
...
cell_option_book vacation::call_option(hero* _hero, int day) {
  ...
  options.push_back(cell_option(L"놀기", [_hero, day]() {
    ...
    _hero->change_status({.energy = 100,
                          .love = 0,
                          .relationship = 0,
                          .self_develop = 0,
                          .study = 0,
...
cell_option_book win_vac::call_option(hero* _hero, int day) {
  vector<cell_option> options;
  options.push_back(cell_option(L"결과 보기", [_hero, day]() { return L""; }));
  return cell_option_book(options);
}
