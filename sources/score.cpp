...
/** 게임을 진행하여 받은 학점을 최종 점수에 합산할 수 있는 점수로 변환해주는
 * 함수이다. grade의 범위에 따라 점수 변환이 다른 식으로 계산된다. 이는 학점에
 * 따라 학사경고, 장학금 짤림 등이 결정되기 때문이다. */
float Score::calc_grade_score(float grade) {
  if (grade < 0)  // grade가 0보다 작으면 runtime error임을 알려준다.
    throw runtime_error("0보다 작으면 안됨");
  //아래에는 grade의 범위에 따라 각각에 따른 점수 변환을 해준다.
  else if (grade < 2.0)
    return (grade) / 2 * 5;
  else if (grade < 2.3)
...
/** 게임 진행 결과 학점을 정해주는 함수이다. 게임에서 얻은 공부점수 만큼 학점을
 * 부여하는데, 학점은 어느 정도의 운이 따르기 때문에 rand()를 이용하여 약간의
 * 오차가 발생하도록 설정하였다. */
...
Score Score::score(const hero& _hero) {
  ...
  // improvement 300
  return {.grade_score =
              static_cast<int>(calc_grade_score(_hero.get_average_grade())),
...
wstring Score::evaluate_total() {
  ...
  if (total > 800) {
    return L"1학년 생활을 굉장히 잘 보내었군요! 당신은 훌륭한 POSTECH의 인재가 "
           L"될 것입니다!";
  } else if (total > 600) {
    return L"높은 점수를 받으셨군요! 축하드립니다! 다음엔 최고를 향해서 도전해 "
           L"보시기 바랍니다!";
  } else if (total > 400) {
    return L"무언가 잊고 사시지는 않았나요? 한가지, 두가지 놓치고 산 것이 큰 "
           L"영향을 미칠 수 있답니다.";
  } else if (total > 200) {
...
wstring Score::evaluate_grade_score() {
  switch (this->grade_score / 100) {
    ...
    case 2:  // 200이상
      return L"오 학점, 좋아 좋아. 일 학년이라면 적당히 놀고 적당히 공부하고. "
             L"딱 좋네요.";
    case 1:  // 100이상
      return L"음...학점이 참...거참.... 일 학년이지만 이건 "
             L"아닌데...음...아닌데...";
    case 0:  // 100미만
      return L"학점이 가관입니다. 혼자 허리케인 같은 일학년 삶을 살았구만요. "
             L"계속되면 안됩니다..";
  }
...
wstring Score::evaluate_relationship() {
  switch (this->relationship / 100) {
    ...
    case 0:  // 100미만
      return L"인간 관계가 약간 부족해요. 일학년 때 놓치면 안될 사람들도 "
             L"많은데!";
  }
...
wstring Score::evaluate_self_develop() {
  switch (this->self_develop / 100) {
    ...
    case 0:  // 100미만
      return L"이건 아닌데.... 진정한 포스테키안이라면 자기 계발은 선택이 "
             L"아니라 필수입니다. 분발하세요!";
  }
