...
hero::hero(const PersonalStatus& status_max, sexuality sex)
    : MAX_STATUS(status_max) {
  this->set_default_initial_status(sex);
}

hero::hero(const PersonalStatus& status_max,
           const PersonalStatus& initial_status, sexuality sex)
    : MAX_STATUS(status_max) {
  this->set_default_initial_status(sex);
...
void hero::take_exam() {
  if (this->_is_spring) {
    if (this->spring_grades.size() < 2)
      this->spring_grades.push_back(
          Score::generate_grade(this->status().study));
    else
      throw runtime_error("시험은 두 번만 봅니다.");
  } else {
    if (this->autumn_grades.size() < 2)
      this->autumn_grades.push_back(
          Score::generate_grade(this->status().study));
    else
      throw runtime_error("시험은 두 번만 봅니다.");
  }
...
//모든 title 초기화
//각종 status 변화를 구현함. 이 때, MAX치보다 크면 MAX치로 되고, 0보다 작아지면
// 0으로 초기화
...
void hero::change_relationship(float relationship,
                               int day) {  // relationship status 변화
  auto result = this->current_status.relationship + relationship * day;
  this->current_status.relationship =
      max(0.0f, min(MAX_STATUS.relationship, result));
}
void hero::change_self_develop(float self_develop, int day) {
  auto result = this->current_status.self_develop + self_develop * day;
  this->current_status.self_develop =
      max(0.0f, min(MAX_STATUS.self_develop, result));
}
...
void hero::change_status(PersonalStatus status_change,
                         PersonalStatus title_effect, int day) {
  ...
  this->change_relationship(
      status_change.relationship * title_effect.relationship, day);
  this->change_self_develop(
      status_change.self_develop * title_effect.self_develop, day);
  this->change_love(status_change.love * title_effect.love, day);
  this->change_stress(status_change.stress * title_effect.stress, day);
}

bool hero::exhausted() const { return (this->current_status.energy == 0); }

void hero::finish_semester() { this->_is_spring = !this->_is_spring; }
